...
firebase.auth().onAuthStateChanged(async function(user) {
  let db = firebase.firestore()
  let apiKey = '13d24610c92cff8be6453cf355b4111b'
  ...
  if (user) {
    ...
    console.log('signed in')    // Ensure the signed-in user is in the users collection
    ...
    document.querySelector('.movies').insertAdjacentHTML('beforeend', `
      <div class="w-1/5 p-4 todo-${todoId} ${opacityClass}">
        <img src="https://image.tmdb.org/t/p/w500${movies.poster_path}" class="w-full">
        <a href="#" class="watched-button block text-center text-white bg-green-500 mt-4 px-4 py-2 rounded">I've watched this!</a>
      </div>
    `)
         ...
         // Signed out
    console.log('signed out')    // Hide the form when signed-out
    document.querySelector('form').classList.add('hidden')    // Initializes FirebaseUI Auth
    let ui = new firebaseui.auth.AuthUI(firebase.auth())    // FirebaseUI configuration
    let authUIConfig = {
      signInOptions: [
